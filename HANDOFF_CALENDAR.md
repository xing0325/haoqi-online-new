...
### ✅ 多轨排序（NLE 式）已落地（2026-06-24）
负责人要的"像剪辑软件多轨、自己决定哪条日程在上轨/下轨"已做：
- `Event` 加 `sort_order int`（迁移 `20260624120000`，已套云库）；FC `eventOrder:"sortOrder,start"`；课表默认 `-1`（最上轨）。
- 编辑表单加「轨道 ⬆上移一轨 / ⬇下移一轨」→ 重复改母系列(`updateSeries`)、单次改本行(`updateEvent`)，持久化 `sort_order`。
- **已浏览器实测**：把"团队站会"上移，DOM 轨序变化且 week→month 重取后仍保持（持久化到库）。
- 同时把"拖动建日程"的固定背景条改回**柔珊瑚选区**（原固定条在背景层会被事件盖住；真正的"条"靠多轨真事件渲染，不再被挡）。
- **仍可选的进阶**：真·垂直拖拽到指定轨（FC daygrid `eventDrop` 只给"落到哪天"，需自定义命中测试，较重）；或 FC premium `resourceTimeline`（**付费**，行=轨/资源，顺带就是愿景里的「空间视角 场地×时间」视图）——**要负责人拍板是否付费**。
...
- **两轮对抗审查（Workflow 多 agent）共修 19 真 bug，全部已上线**：
  - 一轮(13)：NL 午夜/跨时段范围/时长叠加/凌晨/零时长；recurrence **全程按北京 +08:00 帧**展开/描述（修非 CN 时区与早晨 BYMONTHDAY/describe 错位）；updateSeries 改时间清失配 override；override upsert 复活软删行防撞唯一键；recur_until 计末次结束防跨午夜漏取；splitSeries 软删含被移动子行；NL/表单 allDay 透传 + 全天勾选；此次及以后透传 status。
  - 二轮(6)：**[高危回归] updateSeries 仅在"发生时刻网格"(起始/FREQ/BYDAY/BYMONTHDAY)变化才清 override，只改 UNTIL/COUNT 不再误删单次修改**；updateSeries 改规律不依赖 startsAt(回退旧起始)；UNTIL 与展开同帧(+8h)消除 8h 截止错位；scheduleTask 校验事件归属本人；任务勾完成不再永久消失(加"显示已完成"开关)；安排回写失败如实提示。
  - 回归单测共 **57 全绿**；recurring/tasks smoke 10·7 全过。审查脚本在 `…/workflows/scripts/calendar-harden-review*.js`（可 resume 复跑）。
